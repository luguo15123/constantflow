...
Foam::VariableSoftSphere<CloudType>::VariableSoftSphere
(
...
template<class CloudType>
bool Foam::VariableSoftSphere<CloudType>::active() const
{
    return true;
}
...
template<class CloudType>
Foam::scalar Foam::VariableSoftSphere<CloudType>::sigmaTcR
(
...
template<class CloudType>
void Foam::VariableSoftSphere<CloudType>::collide
(
...
)
{
    ...
    scalar alphaPQ =
        0.5
       *(
            cloud.constProps(typeIdP).alpha()
          + cloud.constProps(typeIdQ).alpha()
        );
    ...
    scalar RelUx = cR.x()*cosTheta
                 + sqrt(sqr(cR.y())+sqr(cR.z()))*sinTheta*cos(phi);

    scalar RelUy = cR.y()*cosTheta
                 + (
                     mag(cR)*cR.z()*sin(phi)
                    -cR.x()*cR.y()*cos(phi)
                   )
                 *sinTheta/sqrt(sqr(cR.y())+sqr(cR.z()));
    
    scalar RelUz = cR.z()*cosTheta
                 + (
                     mag(cR)*cR.y()*sin(phi)
                    -cR.x()*cR.z()*cos(phi)
                   )
    ...
    // post relative velocity velocity
    vector postCollisionRelU = vector(RelUx, RelUy, RelUz);
