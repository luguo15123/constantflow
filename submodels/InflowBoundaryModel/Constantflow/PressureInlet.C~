...
Foam::PressureInlet<CloudType>::PressureInlet
(
...
template<class CloudType>
void Foam::PressureInlet<CloudType>::autoMap(const mapPolyMesh& mapper)
{
...
template<class CloudType>
void Foam::PressureInlet<CloudType>::inflow()
{
    CloudType& cloud(this->owner());
    ...
    forAll(patches_, p)
    {
        ...
        forAll(pFA, i)
        {
            ...
            {
                FatalErrorIn ("Foam::PressureInlet<CloudType>::inflow()")
            ...
            // modification to the free stream conditions

            // Read from the dictionnary the numberdensity for the inlet
            scalar nIn = numberDensities_[i];

            // the outlet of the system is vacuum, the number density is 0
            scalar nOut = 0;

            //name your inlet "inlet"
            if ("inlet" == patch.name())
            {
                pFA[i] +=
            ...
            // name your outlet "outlet"
            if ("outlet" == patch.name())
            {
                pFA[i] +=
                    mag(patch.faceAreas())*nOut*deltaT
                   *mostProbableSpeed
                   *(
                        exp(-sqr(sCosTheta))
                      + sqrtPi*sCosTheta*(1 + erf(sCosTheta))
